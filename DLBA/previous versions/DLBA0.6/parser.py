# parser.py - DLBA v0.6 (fixed: skip NEWLINEs inside blocks and central terminator handling)
...
class Parser:
    ...
    # helper: skip any NEWLINE tokens
    def _skip_newlines(self):
        while self.peek() and self.peek().type == 'NEWLINE':
            self.eat('NEWLINE')

    # helper: after a statement, consume a terminator if present; else validate allowed tokens
    def _consume_terminator_or_validate(self):
        ...
        if nxt and nxt.type == 'SEMICOLON':
            ...
            # allowed (end of block) - do not consume
            pass
        elif nxt is None:
            pass
        else:
            raise Exception(f"Expected ';' or newline after statement but got {nxt.type} at {nxt.filename}:{nxt.lineno}")

    # helper: parse a sequence of statements inside a block (until RBRACE)
    def _parse_block_statements(self):
        ...
        while self.peek() and self.peek().type != 'RBRACE':
            ...
            if not self.peek() or self.peek().type == 'RBRACE':
                break
            stmt = self.statement()
            # after each statement inside block, require terminator or RBRACE
    ...
    def parse(self):
        ...
        while self.peek():
            ...
            if not self.peek():
                break
            stmt = self.statement()
            # After statement at top-level, allow SEMICOLON, NEWLINE, RBRACE, or EOF
            self._consume_terminator_or_validate()
    ...
    def statement(self):
        ...
        # import statement: import "file" [as IDENT]
        if tok.type == 'IMPORT':
            ...
            node = Import(path_tok.value, as_name)
            node.lineno = path_tok.lineno; node.filename = path_tok.filename
            return node
        ...
        if tok.type == 'FUNC':
            ...
            node.lineno = func_tok.lineno; node.filename = func_tok.filename
            return node
        ...
        if tok.type == 'RETURN':
            ...
            expr = None
            if self.peek() and self.peek().type not in ('SEMICOLON','NEWLINE','RBRACE'):
                expr = self.expr()
            node = Return(expr)
            node.lineno = getattr(expr, 'lineno', r_tok.lineno) if expr else r_tok.lineno
            node.filename = getattr(expr, 'filename', r_tok.filename) if expr else r_tok.filename
            return node
        ...
        # expression as statement (including call or member access)
        if tok.type in ('IDENT','STRING','NUMBER','LPAREN','LBRACK'):
            expr = self.expr()
            return expr

        # If we get a NEWLINE here it means caller did not skip it; but parse() and block parser skip NEWLINEs
        # So encountering NEWLINE as statement start indicates malformed input.
    ...
    # Expressions with precedence and postfix handling (call, dot, index)
    def expr(self):
        return self.or_expr()
    ...
    def postfix(self):
        ...
        while True:
            ...
            if nxt.type == 'DOT':
                ...
                node.lineno = mem.lineno; node.filename = mem.filename
                continue
            if nxt.type == 'LBRACK':
                self.eat('LBRACK')
                idx = self.expr()
                self.eat('RBRACK')
                node = Index(node, idx)
                node.lineno = getattr(idx, 'lineno', node.lineno)
                node.filename = getattr(idx, 'filename', node.filename)
                continue
            break
        return node
    ...
    def primary(self):
        ...
        if tok.type == 'FALSE':
            t = self.eat('FALSE')
            node = self.eat('FALSE')  # consume
            # above line intentionally consumes, now create node:
            node = Boolean(False)
            node.lineno = t.lineno; node.filename = t.filename
            return node
        if tok.type == 'IDENT':
        ...
        if tok.type == 'LBRACK':
            ...
            node.lineno = lb.lineno; node.filename = lb.filename
            return node
        if tok.type == 'LBRACE':
            lb = self.eat('LBRACE')
            pairs = []
            if self.peek() and self.peek().type != 'RBRACE':
                while True:
                    if self.peek().type == 'STRING':
                        ...
                        key_node.lineno = k_tok.lineno; key_node.filename = k_tok.filename
                    elif self.peek().type == 'IDENT':
                        k_tok = self.eat('IDENT')
                        key_node = String(k_tok.value)
                        key_node.lineno = k_tok.lineno; key_node.filename = k_tok.filename
                    else:
                        raise Exception(f"Expected STRING or IDENT as dict key but got {self.peek().type} at {self.peek().filename}:{self.peek().lineno}")
                    self.eat('COLON')
                    val = self.expr()
