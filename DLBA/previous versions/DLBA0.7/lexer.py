# lexer.py - DLBA lexer (v0.7)
import re

# Global map filename -> list of source lines (for traceback display)
SOURCE_MAP = {}  # filled by tokenize
...
TOKEN_SPEC = [
    ...
    ("PRINT", r"\bprint\b"),
    ("LET", r"\blet\b"),
    # accept True/False and true/false (normalize later)
...
def tokenize(code, filename="<input>"):
    """
    Tokenize `code`. Store original source lines in SOURCE_MAP[filename].
    Ensure NEWLINE handling updates lineno/col exactly once.
    """
    ...
    # iterate matches in cleaned text
    for m in MASTER_RE.finditer(cleaned):
        ...
        # NEWLINE token: handle specially (advance lineno once, reset col)
        if kind == "NEWLINE":
        ...
        # For other tokens, compute whether token contains newlines
        newlines = text.count("\n")
        if newlines:
            # token spans multiple lines: update lineno and compute column after last newline
            last_nl = text.rfind("\n")
            lineno += newlines
            # column becomes length of text after last newline + 1 (1-based column)
            col = len(text) - last_nl
        else:
            # no newlines in token: advance column by token length
            col += len(text)

        # Now create token objects based on kind
        if kind == "FLOAT":
            tokens.append(
            ...
            # store boolean token with boolean value
            tokens.append(
                Token(
            ...
            # whitespace (already accounted in col update)
            continue
        elif kind == "MISMATCH":
            raise Exception(
