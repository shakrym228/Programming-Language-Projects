...
class Parser:
    ...
    def _consume_terminator_or_validate(self):
        ...
        if nxt and nxt.type == "SEMICOLON":
            ...
            raise Exception(
                f"Expected ';' or newline after statement but got {nxt.type} at {nxt.filename}:{nxt.lineno}"
            )
    ...
    # Expressions precedence + postfix (call, dot, index)
    def expr(self):
        return self.or_expr()
    ...
    def primary(self):
            ...
            if self.peek() and self.peek().type != "RBRACE":
                while True:
                    if self.peek().type == "STRING":
                        ...
                        raise Exception(
                            f"Expected STRING or IDENT as dict key but got {self.peek().type} at {self.peek().filename}:{self.peek().lineno}"
                        )
                    self.eat("COLON")
                    val = self.expr()
        ...
        if tok.type == "LPAREN":
            ...
            node.col = getattr(node, "filename", lp.col)
            return node
        raise Exception(
            f"Unexpected token in expression: {tok.type} at {tok.filename}:{tok.lineno}"
        )
